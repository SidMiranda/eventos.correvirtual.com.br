...
- [ ] **Aplicar o cupom na inscrição do atleta** (fatia seguinte da linha acima). Falta o campo "tenho um cupom" na tela de inscrição, a conferência do código, o abatimento em `subscriptions.price` e o valor que vai para o Mercado Pago. Exige `subscriptions.coupon_id` e `subscriptions.discount_amount` — que dão o histórico de uso de graça (hoje o cupom só sabe *quantas* vezes foi usado, não *por quem*). A regra de consumo já existe e está testada em `Coupon::registrarUso()`. **Mexe no fluxo de dinheiro**: não subir junto com o BUG-005 ainda aberto.
