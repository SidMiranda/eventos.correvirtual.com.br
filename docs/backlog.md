...
- [ ] **Zerar o uso da produção antes do lançamento** (pedido do dono em 2026-09-20; lançamento na segunda seguinte): tudo que existe de inscrição e pagamento é teste. Botão pronto: workflow "Zerar uso em produção" (backup na VPS → cópia no R2 → simulação → `APAGAR`). Falta apertar: primeiro sem `APAGAR` e ler a lista, depois com. Os 6 eventos mocados e o "teste de fluxo" ficam por enquanto (o dono pediu para manter o catálogo) — decidir depois da lista, com `base:limpar-testes --evento-de-teste=<slug>` se for o caso.
